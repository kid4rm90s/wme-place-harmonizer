...
- Map Highlights: All places will show with a highlight on the map to indicate data quality and lock status. The highlight color represents the most critical issue found (if any). For details on what each color means and what actions to take, see **[HIGHLIGHTING_GUIDE.md](HIGHLIGHTING_GUIDE.md)**.

- Applying a lock: If a place satisfies quality checks and is complete according to regional standards for its category, the script will lock the place at the appropriate level (determined by PNH regional guidelines). If your editor rank is lower than the lock level required, the script locks it to your rank. The script will never down-lock a place. For complete technical details on severity levels, color meanings, and lock logic, see **[SEVERITY_MAP.md](SEVERITY_MAP.md)**.
